...
- **Ingreso a depósito:** Depósito puede registrar la **ubicación de espera** (opcional/informativa; si se carga, se valida vía puerto, tipo `devoluciones`/`staging`).
- **Procesado:** Depósito cerró el control de **todos** los bultos → se calcula la reconciliación, se registra el **destino** (opcional, ver cierre) y se **emite `devolucion.procesada`**.
...
- Las ubicaciones destino (bueno y malo) son **opcionales/informativas**: NO bloquean el cierre. Si se cargan, se validan por `UbicacionResolverPort.esValidaPara(codigo, tipo)`; si vienen vacías, se guardan `null`. Devoluciones **emite `devolucion.procesada`** con el resultado por ISBN (bueno/malo + destinos, estos últimos posiblemente vacíos).
...
- **Ubicación de espera:** dónde aguardan los bultos mientras se controlan (Ingreso a depósito) → si se carga, valida contra tipo `devoluciones`/`staging`.
- **Ubicación destino:** dónde terminan los libros ya controlados (cierre) → si se carga, valida contra `picking`/`pallet`/`dañados`/`cuarentena`.
- Ambas son **opcionales/informativas** hoy; cuando se cargan pasan por `UbicacionResolverPort` (ningún string de ubicación suelto), nunca por validación ad-hoc.
