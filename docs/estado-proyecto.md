...
- **Calidad**: tests jest 34/34 (`npm test`): seam + ISBN + **permisos por endpoint** (metadata RBAC) + **máquina de estados / propiedad / reglas de observación / reconciliación multi-bulto / circuito e2e del criterio / corrección** sobre un Prisma fake en memoria (`autorizacion.service.spec.ts`). OpenAPI (`/api/docs` + `npm run openapi` → `docs/openapi.yaml`), CSP + rate limiting (login 10/min → 429).
